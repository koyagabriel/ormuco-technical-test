...
class KafkaConnector:
	...
	def _create_topic(self, broker):
		...
		if not self.DEFAULT_TOPIC in admin_client.list_topics().topics:
			new_topic = NewTopic(self.DEFAULT_TOPIC, num_partitions=3, replication_factor=1)
	...
	def publish(self):
		while True:
			...
			if data:
				...
				self._producer.produce(self.DEFAULT_TOPIC, json.dumps(data).encode('utf-8'), callback=self._delivery_report)
